...
public class OVRPlayerController : MonoBehaviour
{
    ...
    //the force used to go down when swiping down.
    public float downForce = 0.5f;
    // how long the player ducks for
    public float duckTime = 1;

    // how long you boost forward
    public float boostTime = 1;

    // how much boosting increases your speed
	...
	protected virtual void Update()
	{
        ...
        if(ducking)
        {
            if(Time.time > duckStartTime + duckTime)
            {
                Controller.height = 2;
                ducking = false;
            }
    ...
    // boosts down in air, also starts ducking
    public bool BoostDown()
    {
        
        
        MoveThrottle -= new Vector3(0, downForce, 0);
        if (!ducking)
        {
            ducking = true;
